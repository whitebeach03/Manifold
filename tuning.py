...
def main():
    ...
    for k in k_list:
        ...
        if data_type == 'mnist':
            ...
            train_loader, val_loader = create_loaders(train_dataset, split_path='data_split_indices_cifar.pkl', batch_size=batch_size, save_if_missing=False)
        elif data_type == 'stl10':
            batch_size    = 64
        ...
        for epoch in range(epochs):
            train_loss, train_acc = train(model, train_loader, criterion, optimizer, device, augment, k)
...
def train(model, train_loader, criterion, optimizer, device, augment, k):
    model.train()
    train_loss = 0.0
    train_acc  = 0.0
    for images, labels in tqdm(train_loader, leave=False):
        ...
        preds = model(images, labels, device, augment, k, aug_ok=True)
