...
def main():
    ...
    augmentations = {
        ...
        "Flipping": transforms.Compose([transforms.RandomApply([transforms.RandomHorizontalFlip(p=1.0)], p=0.5), base_transform]),
        "Cropping": transforms.Compose([transforms.RandomApply([transforms.RandomResizedCrop(size=96, scale=(0.7, 1.0))], p=0.5), base_transform]),
        "Rotation": transforms.Compose([transforms.RandomApply([transforms.RandomRotation(degrees=30)], p=0.5), base_transform]),
        "Translation": transforms.Compose([transforms.RandomApply([transforms.RandomAffine(degrees=0, translate=(0.2, 0.2))], p=0.5), base_transform]),
        "Noisy": transforms.Compose([transforms.RandomApply([transforms.Lambda(lambda x: x + 0.1 * torch.randn_like(x))], p=0.5), base_transform]),
        "Blurring": transforms.Compose([transforms.RandomApply([transforms.GaussianBlur(kernel_size=5)], p=0.5), base_transform]),
        "Random-Erasing": transforms.Compose([transforms.RandomApply([transforms.RandomErasing(p=1.0, scale=(0.1, 0.3), ratio=(0.3, 3.3), value="random")], p=0.5), base_transform])
    }
