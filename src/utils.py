...
def train(model, train_loader, criterion, optimizer, device, augment, num_classes, aug_ok, epochs):
    ...
    for images, labels in tqdm(train_loader, leave=False):
        ...
        if augment == "Default":  
            ...
            preds_mix = model(images, labels, device, augment, aug_ok)
            ...
            if epochs < t_mixup:
                ...
                loss, preds = compute_foma_loss(model, images, labels, k=10, num_classes=num_classes, lambda_almp=1.0, device=device, scaleup=True)
            ...
            if epochs < 100:
                ...
                k = 40
            loss, preds = compute_foma_loss(model, images, labels, augment, k, lambda_almp=1.0, device=device)
        elif augment == "PCA":
