...
def main():
    for i in range(1):
        ...
        # all_features = extract_wrn_features(model=wrn_model, dataset=train_dataset, device=device, batch_size=256, num_workers=4)
        # assert all_features.shape[0] == n_train

        # knn_model = NearestNeighbors(n_neighbors=batch_size, algorithm='auto', metric='euclidean')
