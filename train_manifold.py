...
def train(model, train_loader, criterion, optimizer, device, augment, alpha):
    ...
    for images, labels in tqdm(train_loader, leave=False):
        ...
        mixed_output, target_a, target_b, lam = model(images, labels, mixup_hidden=True,  mixup_alpha=alpha)
        
        lam = lam[0]
        target_a_one_hot = to_one_hot(target_a, 10)
        target_b_one_hot = to_one_hot(target_b, 10)
        mixed_target = target_a_one_hot * lam + target_b_one_hot * (1 - lam)
        loss = mixup_criterion(criterion, mixed_output, target_a, target_b, lam)
        ...
        if augment == 'normal' or 'ours':
            train_acc += accuracy_score(labels.cpu(), mixed_output.argmax(dim=-1).cpu())
        elif augment == 'mixup' or 'mixup_hidden':
            train_acc += (lam * accuracy_score(target_a.cpu(), mixed_output.argmax(dim=-1).cpu())
                          + (1 - lam) * accuracy_score(target_b.cpu(), mixed_output.argmax(dim=-1).cpu()))
        else:
            train_acc = 0
